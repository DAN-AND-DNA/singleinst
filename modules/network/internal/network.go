...
func (network *Network) ModuleConstruct() {
	// 加载配置文件
	network.cfgMgr = viper.New()
	network.cfgMgr.SetConfigName("network")
	network.cfgMgr.SetConfigType("json")
	network.cfgMgr.AddConfigPath("./config/modules/")
	network.cfgMgr.AddConfigPath(cmd.ConfigPath)
	if err := network.cfgMgr.ReadInConfig(); err != nil {
		panic(err)
	}
	...
	network.core.Lock()
	defer network.core.Unlock()
	if err := network.cfgMgr.Unmarshal(network.core); err != nil {
		panic(err)
	}
...
func (network *Network) ModuleRunConfigWatcher() {
	// 监听配置变化
	network.cfgMgr.OnConfigChange(func(e fsnotify.Event) {
		network.core.Lock()
		// 配置更新
		err := network.cfgMgr.Unmarshal(network.core)
		if err != nil {
			panic(err)
		}
		network.core.Unlock()
...
func (network *Network) Start() {
	...
	if network.core.ListenGrpc {
		...
		network.httpRouter.POST("/nanogo/:pkg/:service/:method", gingrpc.GinGrpc(network.ginGrpcOption, true))
...
func (network *Network) Recreate() error {
	...
	if network.core.ListenHttp {
		...
		network.ginGrpcOption.pathToServiceName = network.core.PathToServiceName
